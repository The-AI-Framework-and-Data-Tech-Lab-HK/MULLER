...
# Whitelist mapping scalar schema names to concrete Python types. "Sample" is
# resolved lazily in _scalar_type() because muller.core.sample pulls in the
# storage stack, which imports this module back.
_SCALAR_TYPES = {
    "int": int,
    "float": float,
    "bool": bool,
...
def _validate_object(obj: Any, schema: Union[str, Schema]) -> bool:
    ...
    if typ in scalars:
        return isinstance(obj, _scalar_type(typ))
    try:
        validator = _TYPE_VALIDATORS[typ]
    except KeyError:
        raise InvalidJsonSchemaException(f"Unsupported type: {typ}") from None
...
def validate_json_object(obj: Any, schema: Union[str, Schema]) -> None:
    # bool(ndarray) raises ValueError for multi-element arrays, so empty-ness
    # must be checked explicitly; other falsy objects skip validation as before.
    nonempty = obj.size > 0 if isinstance(obj, ndarray) else bool(obj)
    if nonempty and not _validate_object(obj, schema):
...
def _validate_union(obj: Any, params: List[str]) -> bool:
...
# Whitelist dispatch for composite types (replaces the former dynamic
# globals() lookup). Keys must stay in sync with the `types` list above.
_TYPE_VALIDATORS = {
    "Any": _validate_any,
    "List": _validate_list,
    "Dict": _validate_dict,
