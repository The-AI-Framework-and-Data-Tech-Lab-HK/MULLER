...
# Source: https://github.com/activeloopai/deeplake/blob/94c5e100292c164b80132baf741ef233dd41f3d7/hub/core/meta/encode/base_encoder.py
#
...
class Encoder(ABC):
    ...
    def __init__(self, encoded=None, dtype=ENCODING_DTYPE):
        """
        Base class for custom encoders that allow reading meta information from sample indices without
        decoding the entire encoded state.
        ...
        Layout:
            ...
            Worst case scenario:
                The worst case scenario is when every sample has different meta values. This means the number of rows is
                    equal to the number
                of samples, providing a O(log(N)) lookup.

            Lookup algorithm:
                To get the decoded meta for some sample index, you do a binary search over
                    the column `LAST_SEEN_INDEX_COLUMN`. This will give you
                the row that corresponds to that sample index
                    (since the right-most column is our "last index" for that meta information).
    ...
    def __getitem__(
        ...
        It may return a singular derived value, or a tuple with the derived value and the row index.
        """
        tmp_row_index = self.translate_index(local_sample_index)
        value = self._derive_value(self._encoded[tmp_row_index],
                                   tmp_row_index,
                                   local_sample_index)

        if return_row_index:
            return value, tmp_row_index

        return value

    def __setitem__(self, local_sample_index: int, item: Any):
        """Update the encoded value at a given index.
        Depending on the state, this may increase/decrease the size of the state.
        """
        row_index = self.translate_index(local_sample_index)
        # Sherry: optimize this (vectorize __setitem__ to accept `Index` objects)
        actions = (
        ...
        action_taken_flag = None
        for action in actions:
            if action(item, row_index, local_sample_index):  # type: ignore
                action_taken_flag = action
                break

        if not action_taken_flag:
            raise ValueError(f"Update could not be executed for idx={local_sample_index}, "
                             f"item={str(item)}")

        self.post_process_state(start_row_index=max(row_index - 2, 0))
        self._reset_update_state()
        self.is_dirty = True

    @property
    def array(self):
        """Returns the self._encoded. """ # Sherry: really?
    ...
    @property
    def encoded(self):
        """Returns the self._encoded. """
        return self._encoded

    @encoded.setter
    def encoded(self, value):
        """Set the self._encoded."""
        self._encoded = value

    @classmethod
    def frombuffer(cls, buffer: bytes):
        """Returns from buffer the encoded value. """
    ...
    def check_last_row(self, global_sample_index: int):
        """
        Takes a look at self.last_row and tries to find chunk id without binary search by looking
           at the current and next row.
        """
        arr = self._encoded
    ...
    def translate_index(self, local_sample_index: int) -> int:
        ...
        # Sherry: optimize this (should accept an optional argument for starting point, instead of random binsearch)
        ...
        return int(row_index)  # type: ignore
    ...
    def _derive_value(self, row: np.ndarray, row_index: int, local_sample_index: int):
        """Given a row of `self._encoded`, this method should implement how `__getitem__`
    ...
    def _setup_update(self, item: Any, row_index: int, local_sample_index: int):
        """Setup the state variables for proceeding actions. Used for updating."""
