...
# Source: https://github.com/activeloopai/deeplake/blob/94c5e100292c164b80132baf741ef233dd41f3d7/hub/core/meta/encode/tile.py
#
...
class TileEncoder(MULLERMemoryObject):
    def __init__(self, entries: Optional[Dict[int, Tuple[Tuple[int, ...], Tuple[int, ...]]]] = None,
                 version: Optional[str] = None):
        super().__init__()
        self.is_dirty = False
        self.entries: Dict[int, Tuple[Tuple[int, ...], Tuple[int, ...]]] = entries or {}
        self.version = version or muller.__version__
    ...
    @property
    def nbytes(self):
        len_ver = 1 + len(self.version)
        num_entries = len(self.entries)
        if num_entries == 0:
            return len_ver + 8
        value = next(iter(self.entries.values()))
        num_dim = len(value[0])
        return len_ver + 16 + (num_entries * (8 * (1 + num_dim * 2)))
    ...
    def frombuffer(cls, buffer: bytes, **kwargs):
        ...
        try:
            # Get version length
            version_length = buffer[ofs]
            ofs += 1

            # Get version string
            version = str(buffer[ofs: ofs + version_length], "ascii")
            ofs += version_length
            entries = parse_tile_encoder_entries(buffer, ofs, "little")
            enc = cls(entries, version=version)
        except Exception:
            # backwards compatibility
            entries = parse_tile_encoder_entries(buffer, 0, "big")
            enc = cls(entries)
        enc.is_dirty = False
        return enc

    def register_sample(self, sample: SampleTiles, idx: int):
        """Function to registers a new tiled sample into the encoder.

        Args:
            sample: sample to be registered.
...
def parse_tile_encoder_entries(data, ofs: int, byteorder: str) -> Dict:
    """Function to parse tile encoder entries."""
