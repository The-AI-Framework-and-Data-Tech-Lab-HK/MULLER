...
# Source: https://github.com/activeloopai/deeplake/blob/94c5e100292c164b80132baf741ef233dd41f3d7/hub/core/tiling/serialize.py
#
...
def break_into_tiles(sample: np.ndarray, tile_shape: Tuple[int, ...]) -> np.ndarray:
    ...
    Returns:
        numpy object array of tiles. Each element of the array is a numpy array that is a zero-copy view
        (source = sample array) of the actual tile at the tile coordinate.
    """
    ...
    for tile_coords, _ in np.ndenumerate(tiles):
        ...
        high = low + tile_shape
        idx = tuple(slice(low_slice, high_slice) for low_slice, high_slice in zip(low, high))
...
def serialize_tiles(
    ...
    Args:
        ...
        serialize_func (Callable[[np.ndarray], memoryview]): A function
        that takes a numpy array and returns a memoryview object. This function is used to serialize each tile,
        may be used to compress the tile.
