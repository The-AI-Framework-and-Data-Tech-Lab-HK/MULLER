...
# Source: https://github.com/activeloopai/deeplake/blob/94c5e100292c164b80132baf741ef233dd41f3d7/hub/core/index/index.py
#
...
def slice_length(s: slice, parent_length: int) -> int:
    ...
    total_length = ((stop - start) + step_offset) // step
    return max(0, total_length)


def replace_ellipsis_with_slices(items, ndim: int):
    """Function to replace ellipsis with slices."""
...
class IndexEntry:
    ...
    def indices(self, length: int):
        ...
        def parse_int(i: int) -> int:
            """Converts a potentially negative index to a positive one."""
...
class Index:
    ...
    def __str__(self):
        def eval_value(value):
            """If value is callable, call it and return as a list. Otherwise, return as is."""
            if callable(value):
                return list(value())
            return value
        values = [eval_value(entry.value) for entry in self.values]
    ...
    def __getitem__(
        ...
        if isinstance(item, tuple):
            ...
            for idx, sub_item in enumerate(item):
                ax = new_index.find_axis_idx(offset=idx)
    ...
    def is_slice(self) -> bool:
        """Function to determine whether it is slice."""
        return isinstance(self.values[0].value, slice)

    @classmethod
    def from_json(cls, idxs):
        """Function to idxs from json."""
        entries = []
        for idx in idxs:
    ...
    def find_axis_idx(self, offset: int = 0):
        """Function to returns the index for the nth sub-scriptable axis in the values of an Index.

        Args:
            offset (int): The number of sub-scriptable axes to skip before returning.
        ...
        for idx, entry in enumerate(self.values):
            if entry.subscriptable():
                if matches_int == offset:
                    return idx
                matches_int += 1
        return None

    def compose_at(self, item: IndexValue, i: Optional[int] = None):
        """Function to returns a new Index representing the addition of an IndexValue,
        or the composition with a given axis.

        Examples:
            >>> Index([slice(None), slice(None)]).compose_at(6)
            Index([slice(None), slice(None), 6])

            >>> Index([slice(None), slice(5, 10), slice(None)]).compose_at(3, 1)
            Index([slice(None), 8, slice(None)])

        Args:
            item (IndexValue): Value to append or compose with the Index.
            i (int, optional): Axis to compose with the given item.
        ...
        new_vals = self.values[:i] + [self.values[i][item]] + self.values[i + 1 :]
        return Index(new_vals)
    ...
    def subscriptable_at(self, i: int) -> bool:
        """Function to return the subscriptable at."""
        try:
    ...
    def length_at(self, i: int, parent_length: int) -> int:
        """Function to return the length at."""
        try:
