...
from muller.util.diff import sanitize_commit, get_lowest_common_ancestor, get_changes_and_messages, \
    get_all_changes_string, get_tensor_changes_for_id, get_dataset_changes_for_id, get_commits_and_messages, \
    get_all_commits_string, calcul_range, handle_append_ranges, handle_update_ranges, handle_delete_ranges, \
    generate_add_values, generate_update_values, generate_delete_values
...
from muller.util.merge import merge_detect, get_node_tensors, direct_detect
...
def merge(
    ...
    ds.storage.autoflush = False
    try:
        muller.util.merge.merge(ds, target_id, conflict_resolution, delete_removed_tensors, force)
    finally:
        if locked:
