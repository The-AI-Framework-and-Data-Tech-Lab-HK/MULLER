...
* For additional optional arguments and advanced usage of these APIs, please refer to the API documentation: [`muller.empty()`](../api/dataset-creation.md#mullerempty) and [`muller.dataset()`](../api/dataset-creation.md#mullerdataset).
...
```
Currently, MULLER supports the following tensor column types (`htype`), compression formats (`sample_compression`), and default data types (`dtype`). For detailed information about each htype, see the [Htypes Reference](../api/htypes.md).
...
- **Note 3:** In addition to `htype`, `sample_compression`, and `dtype`, additional parameters can be specified when creating tensor columns for advanced use cases, such as [`chunk_compression`](../api/dataset-methods.md#dscreate_tensor) for chunk-level compression, and `coords` for `bbox` columns. Refer to the [`create_tensor()`](../api/dataset-methods.md#dscreate_tensor) API documentation and [Htypes Reference](../api/htypes.md) for details.
- **Note 4:** Tensor column names must not contain any MULLER reserved keywords or any Python reserved keywords.
  ...
  ```
- For more detailed usage, please refer to [`create_tensor()`](../api/dataset-methods.md#dscreate_tensor). We plan to support additional `htype`s and provide clearer guidelines for tensor column creation in future releases. This documentation will be updated accordingly.
...
```
* Refer to [`Tensor.append()`](../api/tensor.md#tensorappend) and [`Tensor.extend()`](../api/tensor.md#tensorextend) for detailed usage.
...
For a detailed explanation, see [Advanced Operations: Using with for Better Write Performance](5_advanced_operations.md#2-use-with-for-better-write-performance).
...
See [Advanced Operations: Recommendations for Ingesting Large-Scale Data](5_advanced_operations.md#1-recommendations-for-ingesting-large-scale-data-into-muller-datasets) for details.
...
[`muller.from_file()`](../api/dataset-creation.md#mullerfrom_file).
...
>>> ds = muller.from_dataframes(dataframes, "my_muller_dataset/", schema=schema_1, workers=0)
>>> schema_2 = ... # The same example as in Option 1
>>> ds = muller.from_dataframes(dataframes, "my_muller_dataset/", schema=schema_2, workers=0)
```
For detailed API specifications and file format examples, please refer to
[`muller.from_dataframes()`](../api/dataset-creation.md#mullerfrom_dataframes).
...
[`muller.from_csv()`](../api/dataset-creation.md#mullerfrom_csv) and [`ds.add_data_from_csv()`](../api/dataset-creation.md#dsadd_data_from_csv).
